...
class LaneNetCluster(object):
    ...
    def _cluster(prediction, bandwidth):
        ...
        # log.info('聚类簇个数为e: {:d}'.format(num_clusters))
    ...
    def _get_lane_area(binary_seg_ret, instance_seg_ret):
        ...
        idx = np.where(binary_seg_ret > np.max(binary_seg_ret)/3)
    ...
    def get_lane_mask(self, binary_seg_ret, instance_seg_ret):
        ...
        new_lane_embedding_feats = lane_embedding_feats[:, np.newaxis]
        # band = estimate_bandwidth(new_lane_embedding_feats, quantile=0.2, n_samples=200)
        band = 0.16
        num_clusters, labels, cluster_centers = self._cluster(new_lane_embedding_feats, bandwidth=band)
        ...
        # coord_lanes = []
        # for i in range(num_clusters):
        #     gray = self._gray_map[i]
        #     gray_index = np.where(gray_image == gray)
        #     for j in range(290, 590, 20):
        #         coord_lane = []
